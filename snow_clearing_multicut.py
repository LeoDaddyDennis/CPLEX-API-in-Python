...
Multi-Cut Formulation and Solution
"""
...
stg1_var_names = ['sum salt', 'sum fuel', 'theta_cold', 'theta_warm']
stg1_objective = [summer_cost['salt'], summer_cost['fuel'], 
                  winter_prob['cold'], winter_prob['warm']]
stg1_lower_bounds = [0.0, 0.0, -m, -m]
...
def solve_relaxed(iteration):
    ...
    #theta_hat_cold[iteration] = dec_vars[2]
    #theta_hat_warm[iteration] = dec_vars[3]
...
def solve_sub(iteration, curr_sum_salt, curr_sum_fuel):
    ...
    # get first stage thetas
    theta_hat_cold[iteration] = stg1_snow_clearing.solution.get_values()[2]
    theta_hat_warm[iteration] = stg1_snow_clearing.solution.get_values()[3]
    ...
    # get z_hat
    z_hat = (-winter_prob['cold']*theta_hat_cold[iteration] -
            winter_prob['warm']*theta_hat_warm[iteration] +
...
def add_cut(iteration, dual_vars):
    ...
    Creates the cut gradients and intercepts for each scenario.
    Returns the cut gradients and intercepts.
    '''
    
    cut_gradient_salt_cold = dual_vars[2]
    cut_gradient_salt_warm = dual_vars[6]                    
    cut_gradient_fuel_cold = dual_vars[3]
    cut_gradient_fuel_warm = dual_vars[7]   
    cut_intercept_cold = (dual_vars[0]*truck_days + 
                          dual_vars[1]*plowing_req['cold'])
    cut_intercept_warm = (dual_vars[4]*truck_days + 
                          dual_vars[5]*plowing_req['warm'])
    return [cut_gradient_salt_cold, cut_gradient_salt_warm, 
            cut_gradient_fuel_cold, cut_gradient_fuel_warm, 
            cut_intercept_cold, cut_intercept_warm]

def multi_cut():
    '''
    Ties together all functions as steps in the multi cut algorithm.
    '''
    k = 0
    while not check_tolerance(k):
        ...
        # how is it getting more in tolerance if 1st stage variables don't
        # change?
        if check_tolerance(k) == True:
            print('Solution found!')
            break
        else:
            cut_info = add_cut(k, dual_vars)
            # adds the cuts to the master problem
            stg1_snow_clearing.linear_constraints.add(
                    lin_expr = [[['sum salt', 'sum fuel', 'theta_cold'], 
                                [-cut_info[0], -cut_info[2], 1]],
                                [['sum salt', 'sum fuel', 'theta_warm'],
                                 [-cut_info[1], -cut_info[3], 1]]],
                    senses = ['G', 'G'],
                    rhs = [cut_info[4], cut_info[5]],
                    names = ['cut cold ' + str(k), 'cut warm ' + str(k)])
            # check to ensure algorithm doesn't run forever is something
            # is wrong
            if k > 500:
                break
